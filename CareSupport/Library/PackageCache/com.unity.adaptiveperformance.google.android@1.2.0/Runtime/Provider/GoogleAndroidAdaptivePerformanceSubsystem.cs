        ...
        public class GoogleAndroidAdaptivePerformanceSubsystemProvider : APProvider, IApplicationLifecycle, IDevicePerformanceLevelControl
        {
            ...
            internal class NativeApi
            {
                ...
                public bool SetupGameState()
                {
                    ...
                    try
                    {
                        AndroidGame.SetGameState(true, 0, 0, 0);
                        success = true;
                    }
                    catch (Exception ex)
                    {
                        ADPFLog.Debug($"[Exception] RegisterListener() Game State failed with {ex}!");
                    }

                    if (!success)
                        ADPFLog.Debug($"failed to register game state");
                ...
                public bool Initialize()
                {
                    ...
                    try
                    {
                        ...
                        if (TryParseVersion(GetVersion(), out initVersion))
                        {
                            ...
                            if (initVersion >= new Version(13, 0))
                            {
#if UNITY_2023_1_OR_NEWER
                                SetupGameState();
#endif
